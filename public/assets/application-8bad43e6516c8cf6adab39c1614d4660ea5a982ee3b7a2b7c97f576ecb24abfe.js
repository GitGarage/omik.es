...
function draw(r, repeat, clear)
{
    ...
    var curvey = [];
    $.each(sorted_sides, function(a,b){if(b[4] == 4) curvey.push(b)});
    if (forward(curvey[0]) > 0)
    {
...
function drawCurves(repeat)
{
    ...
    for (var z = 0; z < 4; z++)
    {
        ...
        curves[z] = {};
        curves[z].x = 0 - this_shape[z%4].x * ((this_shape[z%4].z + 8) / 10) * 0.600;
        curves[z].y = this_shape[z%4].y * ((this_shape[z%4].z + 8) / 10) * 0.600;
        curves[z].z = -1;
    }
}
