...
function drawCurves(repeat)
{
    ...
    if (repeat == 10)
    {
        for (var i = 10; i > 0; i--)
        {
            that_shape = i == 10 ? rotate_ys(n2, rotate_xs(n1, hash)) : rotate_ys(n2, rotate_xs(n1, curves[i+1]));
            curves[i] = [];
            for (var z = 0; z < 4; z++)
            {
                curves[i][z%4] = {};
                curves[i][z%4].x = 0 - that_shape[z%4].x * ((that_shape[z%4].z + 8) / 10) * 0.600;
                curves[i][z%4].y = that_shape[z%4].y * ((that_shape[z%4].z + 8) / 10) * 0.600;
                curves[i][z%4].z = -1;
            }
        }
    }

    var this_shape = curves[repeat];
