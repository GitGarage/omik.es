...
function drawBack()
{
    ...
    for (var zed = 1; zed < 4; zed++)
    {
        ctx.lineTo((this_shape[(z+zed)%4].x * (half / 1.500)) * ((this_shape[(z+zed)%4].z + 8) / 10) + half),
        (this_shape[(z+zed)%4].y * (half / 1.500)) * ((this_shape[(z+zed)%4].z + 8) / 10) + half);
    }
    ctx.closePath();
    ctx.fill();
}
