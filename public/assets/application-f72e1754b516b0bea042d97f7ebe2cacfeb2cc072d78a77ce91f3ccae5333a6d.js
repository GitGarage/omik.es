    ...
    for (var s = 0; s < 6; s++)
    {
        ...
        if (side[4] == 1)
        {
            if (repeat > 0)
            {
                ...
                for (var l = 0; l < 5; l++)
                {
                    var corner = angular.copy(new_coords[l%4]);
                    switch(l) {
                        case 0:
                            ctx.moveTo((corner.x * (half / 1.500)) * ((corner.z + 8) / 10) + half,
                                (corner.y * (half / 1.500)) * ((corner.z + 8) / 10) + half);
...
function drawCurves(repeat)
{
    ...
    var this_shape = repeat == 10 ? rotate_ys(n2, rotate_xs(n1, hash)) : rotate_ys(n2, rotate_xs(n1, new_curves));
    ...
    for (var z = 0; z < 4; z++)
    {
        ...
        new_curves[z] = {};
        new_curves[z].x = 0 - this_shape[z%4].x * ((this_shape[z%4].z + 8) / 10) * 0.600;
        new_curves[z].y = this_shape[z%4].y * ((this_shape[z%4].z + 8) / 10) * 0.600;
        new_curves[z].z = -1;
    }
}
