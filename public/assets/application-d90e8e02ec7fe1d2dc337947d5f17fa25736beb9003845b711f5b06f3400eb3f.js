...
function draw(r, repeat, clear)
{
    ...
    for (var s = 0; s < 6; s++)
    {
        ...
        for (var l = 0; l < 5; l++)
        {
            ...
            new_coords[l % 4] = {};
            new_coords[l % 4].x = 0 - corner.x * ((corner.z + 8) / 10) * 0.600;
            new_coords[l % 4].y = corner.y * ((corner.z + 8) / 10) * 0.600;
            new_coords[l % 4].z = -1;
        }
        ctx.closePath();
        ctx.stroke();
...
function drawCurves(repeat)
{
    ...
    if (repeat == 1)
    {
        this_shape = rotate_ys(n2, rotate_xs(n1, hash));
    }
    else
    {
        this_shape = rotate_ys(n2, rotate_xs(n1, angular.copy(hold_side_1)));;
    }
    ...
    for (var z = 0; z < 4; z++)
    {
        ...
        {
            hold_side_1[(z+0)%4] = {};
            hold_side_1[(z+0)%4].x = 0 - this_shape[(z+0)%4].x * ((this_shape[(z+0)%4].z + 8) / 10) * 0.600;
            hold_side_1[(z+0)%4].y = this_shape[(z+0)%4].y * ((this_shape[(z+0)%4].z + 8) / 10) * 0.600;
            hold_side_1[(z+0)%4].z = -1;
        }
    }
}
...
app.controller("mainController", ['$scope', function ($scope) {
  ...
  s.init = function() {
    ...
    canvas.focus();
    draw(rotate_y(n2, rotate_x(n1)), 1, true);
